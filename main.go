...
var (
  ...
  dbInfoUrl = "http://localhost:8080/api/dbLinkInfo/getAll"
  dsnMap = make(map[int]string)
...
func NewExporter(dbId uint64,dsn string) *Exporter {
...
func main() {
  ...
  response,err := http.Get(dbInfoUrl);
  if err != nil {
    panic(errors.New("无法获取数据库连接信息"))
  }
  dbTxtInfo, err := ioutil.ReadAll(response.Body)
  if err != nil {
    panic(errors.New("数据库连接信息读取失败"))
  }

  dbList :=  []DbLinkInfo{}
  err = json.Unmarshal(dbTxtInfo, &dbList)
  if err != nil {
    panic(errors.New("数据库连接信息JSON转换失败"))
  }
  ...
  for _,v := range dbList {
    dsn := fmt.Sprintf("%s/%s@%s:%d/%s",v.UserName,v.Password,v.Ip,v.Port,v.ServiceName)
    fmt.Println("dsn=",dsn)
    //dsnMap[v.Id] = dsn

    exporter := NewExporter(uint64(v.Id),dsn)
  ...
  //http.Handle("/test", promhttp.HandlerFor(registry, promhttp.HandlerOpts{}))
